...
namespace pruebas
{
    public partial class Letter_S : Form
    {
        public Letter_S()
        {
            InitializeComponent();
        ...
        {
            //var p2b = panel1.Controls[0].Controls.OfType<PictureBox>().ToList();
            //var p2b1 = p2b.Single(x => x.Name == "00");
            
            var Mcon = panel1.Controls[0].Controls;
            var fi = Mcon.OfType<Control>();

            if (!fi.ToList().All(x => x.Enabled == true))
            {
                fi.ToList().ForEach(x => x.Enabled = true);
            }
            else
            {
                fi.ToList().ForEach(x => x.Enabled = false);
            }
